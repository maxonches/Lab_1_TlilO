...
func NewStep(
	...
	prevStep *BrownRobinsonStep) *BrownRobinsonStep {
		...
		if prevStep != nil {
			...
			curLowerGameCost = mat.Min(aGain) / float64(stepNum)
...
func (s *BrownRobinsonStep) String() string {
	return fmt.Sprintf("%d | %d | %d | %v | %v | %.3f | %.3f | %.3f",
		s.stepNumber, s.aChoice, s.bChoice, s.aGain.RawVector().Data, s.bGain.RawVector().Data,
		s.minUpperGameCost, s.maxLowerGameCost, s.epsilon)
}
